...
def analyse_ppe(detections):
    ...
    vests   = [d for d in detections if d["label"] in ("vest", "safety vest")]
...
def _label(img, text, x, y, color, scale=0.52, thick=1):
    ...
    y = max(y, th + 4)
    cv2.rectangle(img, (x, y - th - 4), (x + tw + 6, y + 2), (15, 15, 15), -1)
...
def draw_detections(frame, ppe_analysis, raw_ppe_dets, smoking_dets):
    ...
    for d in raw_ppe_dets:
        if d["label"] == "helmet":
            x1,y1,x2,y2 = d["box"]
            cv2.rectangle(out,(x1,y1),(x2,y2), COLORS["safe"], 2)
            _label(out, f"Helmet {d['conf']:.0%}", x1, max(y1-2,14),
                   COLORS["safe"], 0.48)
        elif d["label"] in ("vest","safety vest"):
            x1,y1,x2,y2 = d["box"]
            cv2.rectangle(out,(x1,y1),(x2,y2), COLORS["safe"], 2)
    ...
    cv2.putText(out, ts, (10, H-10),
                cv2.FONT_HERSHEY_SIMPLEX, 0.48, COLORS["grey"], 1, cv2.LINE_AA)
