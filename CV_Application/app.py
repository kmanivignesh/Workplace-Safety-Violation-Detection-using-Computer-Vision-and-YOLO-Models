...
def _load_models():
    ...
    if ppe_model:
        ...
        logger.warning("⚠️  PPE model not found – detection will be skipped.")
    if smoking_model:
        logger.info("✅ Smoking model ready.")
    else:
        logger.warning("⚠️  Smoking model not found – detection will be skipped.")
...
def _run_both_models(frame: np.ndarray):
    ...
    if ppe_model:
        return process_frame(frame, ppe_model, ppe_model)   # smoke will return nothing
    if smoking_model:
